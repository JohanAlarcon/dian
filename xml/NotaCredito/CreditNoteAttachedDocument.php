...
class CreditNoteAttachedDocument
{
    ...
    public function generateXML(array $data, array $data_fac)
    {
        ...
        try {
            ...
            $xmlCreditNote          = Helper::getRawXMLCreditNote($data, $data_fac,$notaCreditoReferenciada);
            $xmlApplicationResponse = Helper::getRawXMLCreditNoteApplicationResponse($data, $data_fac);

            $externalReferenceInvoice             = new ExternalReference('text/xml', 'UTF-8', "$xmlCreditNote");
            ...
            $printXmlCreditNote = true;
            $printXmlAttachedDocument = false;

            if ($printXmlAttachedDocument) {

                $fileName = 'CreditNoteAttachedDocumentGenerate.xml';
            ...
            if ($printXmlCreditNote) {

                $fileName = 'CreditNoteGenerate.xml';
                $filePath = __DIR__ . "/xmlGenerados/$fileName";
                file_put_contents($filePath, $xmlCreditNote);
            ...
            if ($printXmlApplicationResponse) {
                ...
                $fileName = 'CreditNoteAplicationResponseGenerate.xml';
                $filePath = __DIR__ . "/xmlGenerados/$fileName";
