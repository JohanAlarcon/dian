...
class InvoiceTransformer
{
    ...
    public function __construct(array $data, array $data_fac)
    {
        ...
        require_once __DIR__ . '/../../Entities/InvoiceControl.php';
        ...
        require_once __DIR__ . '/../../Entities/OrderReference.php';
        ...
        require_once __DIR__ . '/../../Entities/LegalMonetaryTotal.php';
        require_once __DIR__ . '/../../Entities/InvoiceLine.php';
    ...
    public function generateXML()
    {
        try {
            ...
            $orderReference = new OrderReference($ID, $issueDate);
            ...
            // Crear Delivery
            $delivery = $this->createDelivery(); //Pendiente 1 etiqueta (98%)
            ...
            $paymentMeans = new PaymentMeans($ID, $paymentMeansCode, $paymentDueDate);
            ...
            // Crear LegalMonetaryTotal
            $legalMonetaryTotal = $this->createLegalMonetaryTotal(); //Pendiente
            ...
            $DueDate = $this->dataProcesarVP['fechaVencimiento'];
            $InvoiceTypeCode = "01";
            $Note = "";
    ...
    private function createUblExtensions()
    {
        ...
        try {
            ...
            // AuthorizedInvoices
            $authorizedInvoices = new AuthorizedInvoices($Prefix, $From, $To);

            // AuthorizationPeriod
            $authorizationPeriod = new AuthorizationPeriod($StartDate, $EndDate);

            // InvoiceControl
            $InvoiceControl = new InvoiceControl($resolucion_dian, $authorizationPeriod, $authorizedInvoices);
    ...
    private function createAccountingSupplierParty()
    {
        ...
        $telephone      = $dataEmpresa[0]['telefono'];
        $electronicMail = $dataEmpresa[0]['email'];

        $contact = new Contact($telephone, $electronicMail);
        //$corporateRegistrationScheme = new CorporateRegistrationScheme("SETG");
        ...
        $partyLegalEntity = new PartyLegalEntity($registrationName, $companyID);
        ...
        Validator::validateRequired($line, 'Direccion de la Empresa');
        ...
        $addressPhysicalLocation = new Address("11001", "Bogota, D.c. ", "110521", "Bogota", "11", $addressLine, $country);
        ...
        return new AccountingSupplierParty($additionalAccountID, $party);
    }
    ...
    private function createAccountingCustomerParty()
    {
        ...
        $telephone      = $dataCliente[0]['telefono'];
        ...
        //$corporateRegistrationScheme = new CorporateRegistrationScheme($ID);
        ...
        /**
         * Grupo para informar Direccion fiscal del emisor 
         */
        ...
        return new AccountingCustomerParty($additionalAccountID, $party,$includeIndustryClassificationCode = false);
    }

    /**
     * Crea la información de Delivery.
     */
    private function createDelivery()
    {

        $dataCliente  = $this->model->getDataCliente($this->cliente_id);
        $line = $dataCliente[0]['direccion'];
        ...
        $deliveryAddress = new Address($ID, $CityName, $PostalZone, $CountrySubentity, $countrySubentityCode, $addressLine, $country);

        $ActualDeliveryDate = date('Y-m-d', strtotime(trim($this->dataProcesarVP['ordenDeCompra'][0]['fecha'])));

        return new Delivery($ActualDeliveryDate, $deliveryAddress);
    }
    ...
    private function createTaxTotal()
    {
        ...
        $taxSubtotal = new TaxSubtotal($taxableAmount, $taxableAmountAttributes, $TaxAmount, $taxAmountAttributes, $taxCategory);

        $TaxAmount = $this->dataProcesarVP['totalMonto'];

        Validator::validateRequired($TaxAmount, 'Monto total del impuesto');

        return new TaxTotal($TaxAmount, $taxAmountAttributes, $taxSubtotal);
    }

    /**
     * Crea la información de LegalMonetaryTotal.
     */
    private function createLegalMonetaryTotal() //
    {
    ...
    private function createInvoiceLine(
        ...
        $standardItemIdentification = new StandardItemIdentification($ID, ["schemeID" => $schemeID]);
        ...
        $item = new Item($Item, $standardItemIdentification, $additionalItemProperty1, $additionalItemProperty2, $additionalItemProperty3, $withAdditionalProperties);

        $lineExtensionAmountAttributes = ["currencyID" => "COP"];
        $invoicedQuantityAttributes = ["unitCode" => $unitCode];
        $IDAttributes = ["schemeID" => $IDScheme];
        ...
        if ($withTax) {
            ...
            $taxTotal = new TaxTotal($valorTOTALImp, $taxAmountAttributes, $taxSubtotal);
        }
        ...
        return new InvoiceLine(
            ...
            $withAdditionalProperties ? 'false' : null,
            $taxTotal,
            $item,
            $price
        );
