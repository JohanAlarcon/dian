...
class DebitNoteAttachedDocument
{
    ...
    public function generateXML(array $data, array $data_fac)
    {
        ...
        try {
            ...
            $notaDebitoReferenciada= true;
            $xmlDebitNote          = Helper::getRawXMLDebitNote($data, $data_fac,$notaDebitoReferenciada);
            $xmlApplicationResponse = Helper::getRawXMLDebitNoteApplicationResponse($data, $data_fac);

            $externalReferenceInvoice             = new ExternalReference('text/xml', 'UTF-8', "$xmlDebitNote");
            ...
            $printXmlDebitNote = true;
            $printXmlAttachedDocument = false;

            if ($printXmlAttachedDocument) {

                $fileName = 'DebitNoteAttachedDocumentGenerate.xml';
            ...
            if ($printXmlDebitNote) {

                $fileName = 'DebitNoteGenerate.xml';
                $filePath = __DIR__ . "/xmlGenerados/$fileName";
                file_put_contents($filePath, $xmlDebitNote);
            ...
            if ($printXmlApplicationResponse) {
                ...
                $fileName = 'DebitNoteAplicationResponseGenerate.xml';
                $filePath = __DIR__ . "/xmlGenerados/$fileName";
